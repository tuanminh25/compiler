...
# Class name for `java` invocation: convert path separators to dots, strip leading ./
CLASS=$(echo "$BASENAME" | sed 's|^\./||; s|/|.|g')
...
java VC.vc "$TEST_VC" || { echo "FAIL: VC compiler crashed on $TEST_VC"; exit 1; }
