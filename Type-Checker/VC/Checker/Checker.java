...
public final class Checker implements Visitor {
    ...
    private void declareVariable(Ident ident, Decl decl) {
        idTable.retrieveOneLevel(ident.spelling).ifPresent(entry ->
            reporter.reportError(ErrorMessage.IDENTIFIER_REDECLARED.getMessage(),
                                 ident.spelling, ident.position)
        );
    ...
    /**
     * Checks an array initialiser against the declared array type.
     * Reports *13 (wrong element type), *16 (excess elements).
     * Handles int->float coercion for valid elements.
     */
    private void checkArrayInit(ArrayInitExpr aie, ArrayType at,
                                String varName, SourcePosition declPos, Object o) {
        Type elemType = at.T;

        // Determine declared size (-1 means unsized, i.e., EmptyExpr)
        int declaredSize = -1;
        if (at.E instanceof IntExpr ie) {
            try { declaredSize = Integer.parseInt(ie.IL.spelling); } catch (NumberFormatException ignored) {}
        }

        int idx = 0;
        ASTList<Expr> list = aie.IL;
        while (!list.isEmpty()) {
            Expr elem = list.getHead();
            Type elemActual = (Type) elem.visit(this, o);

            if (!elemActual.isErrorType()) {
                if (elemType.isFloatType() && elemActual.isIntType()) {
                    // coerce int element to float — replace in list
                    list.setHead(new UnaryExpr(new Operator("i2f", elem.position), elem, elem.position));
                } else if (!elemType.equals(elemActual)) {
                    reporter.reportError(ErrorMessage.WRONG_TYPE_FOR_ARRAY_INITIALISER.getMessage(),
                                         "at position " + idx, elem.position);
                }
            }
        ...
        // *16: excess elements
        if (declaredSize >= 0 && idx > declaredSize) {
            reporter.reportError(ErrorMessage.EXCESS_ELEMENTS_IN_ARRAY_INITIALISER.getMessage(),
                                 varName, declPos);
        }
    }

    /** Returns the VC type name as a string, used in conditional error messages. */
    private String typeName(Type t) {
        if (t.isIntType())     return "int";
        if (t.isFloatType())   return "float";
        if (t.isBooleanType()) return "boolean";
        if (t.isStringType())  return "string";
        if (t.isVoidType())    return "void";
        if (t.isArrayType())   return "array";
        return "error";
    }
    ...
    public Object visitForStmt(ForStmt ast, Object o) {
        ...
        if (!exprType2.isErrorType() && !exprType2.isBooleanType()) {
            reporter.reportError(ErrorMessage.FOR_CONDITIONAL_NOT_BOOLEAN.getMessage(),
                                 typeName(exprType2), ast.E2.position);
        }
        ast.S.visit(this, o);
        inLoop--;
        return null;
    ...
    public Object visitStmtList(StmtList ast, Object o) {
        ...
        // break/continue are only terminal (can make following code unreachable)
        // when they are valid, i.e., inLoop > 0.
        // Outside a loop they are errors themselves and are not treated as terminal.
        boolean isTerminal = (ast.getHead() instanceof ReturnStmt)
            || ((ast.getHead() instanceof BreakStmt || ast.getHead() instanceof ContinueStmt)
                && inLoop > 0);

        if (isTerminal && ast.getNext() instanceof StmtList) {
            reporter.reportError(ErrorMessage.STATEMENTS_NOT_REACHED.getMessage(),
                                 "", ast.getNext().position);
        }
    ...
    public Object visitAssignExpr(AssignExpr ast, Object o) {
        ...
        // 1. Syntactically invalid lvalue (e.g., 1 + 2 = r)
        if (!(ast.E1 instanceof VarExpr || ast.E1 instanceof ArrayExpr)) {
            ast.type = StdEnvironment.errorType;
            reporter.reportError(ErrorMessage.INVALID_LVALUE_IN_ASSIGNMENT.getMessage(),
                                 "", ast.position);
            return ast.type;
        }

        // 2. VarExpr that resolves to a function (also invalid lvalue, e.g., f = ...)
        if (ast.E1 instanceof VarExpr ve) {
            SimpleVar sv = (SimpleVar) ve.V;
            if (sv.I.decl instanceof FuncDecl fd2) {
                ast.type = StdEnvironment.errorType;
                reporter.reportError(ErrorMessage.INVALID_LVALUE_IN_ASSIGNMENT.getMessage(),
                                     fd2.I.spelling, ast.position);
                return ast.type;
            }
        }

        // 3. Error type passthrough — suppress cascading errors
        if (type1.isErrorType() || type2.isErrorType()) {
            ast.type = StdEnvironment.errorType;
            return ast.type;
        }

        // 4. int→float coercion on RHS
        if (type1.isFloatType() && type2.isIntType()) {
            ...
            ast.type = StdEnvironment.errorType;
            reporter.reportError(ErrorMessage.INCOMPATIBLE_TYPE_FOR_ASSIGNMENT.getMessage(),
                                 "", ast.position);
        }
        return ast.type;
    }
    ...
    public Object visitCallExpr(CallExpr ast, Object o) {
        ...
        while (!formals.isEmpty() && !actuals.isEmpty()) {
            ...
            // For VarExpr actuals: call visitSimpleVar directly to get the raw
            // declared type (including ArrayType) without triggering *11 in
            // visitVarExpr. Arrays can validly be passed to array parameters.
            Type actualType;
            if (actual.E instanceof VarExpr ve && ve.V instanceof SimpleVar sv) {
                actualType = (Type) sv.visit(this, o);
                ve.type = actualType; // keep decorated AST consistent
            } else {
                actualType = (Type) actual.E.visit(this, o);
            }

            if (!actualType.isErrorType()) {
                if (formalType.isArrayType() && actualType.isArrayType()) {
                    // Array-to-array: arrays cannot use equals(), so compare element types
                    Type formalElem = ((ArrayType) formalType).T;
                    Type actualElem = ((ArrayType) actualType).T;
                    if (!formalElem.equals(actualElem)) {
                        reporter.reportError(ErrorMessage.WRONG_TYPE_FOR_ACTUAL_PARAMETER.getMessage(),
                                             formal.I.spelling, actual.E.position);
                    }
                } else if (formalType.isFloatType() && actualType.isIntType()) {
    ...
    public Object visitArrayExpr(ArrayExpr ast, Object o) {
        ...
        ast.type = ((ArrayType) arrayType).T;
        return ast.type;
    }
    ...
    public Object visitArrayInitExpr(ArrayInitExpr ast, Object o) {
        // Logic is driven by visitLocalVarDecl/visitGlobalVarDecl via checkArrayInit.
        // This stub is a no-op when called standalone.
    ...
    public Object visitBinaryExpr(BinaryExpr ast, Object o) {
        ...
        // Suppress cascading errors — don't report *9 when operands already errored
        ...
        if (op.equals("+") || op.equals("-") || op.equals("*") || op.equals("/")) {
            if (t1.isIntType() && t2.isIntType()) {
                ...
                ast.type = StdEnvironment.errorType;
                // Report at the whole expression position; pass operator as tokenName
                reporter.reportError(ErrorMessage.INCOMPATIBLE_TYPE_FOR_BINARY_OPERATOR.getMessage(),
    ...
    public Object visitUnaryExpr(UnaryExpr ast, Object o) {
        ...
        if (op.equals("i2f")) {
            // Injected coercion node — always valid
            ast.type = StdEnvironment.floatType;
        } else if (op.equals("+") || op.equals("-")) {
            if (eType.isIntType() || eType.isFloatType()) {
                ...
                ast.type = StdEnvironment.errorType;
                // Report at the whole unary-expr position; pass operator as tokenName
                reporter.reportError(ErrorMessage.INCOMPATIBLE_TYPE_FOR_UNARY_OPERATOR.getMessage(),
                                     ast.O.spelling, ast.position);
            }
        } else if (op.equals("!")) {
    ...
    public Object visitVarExpr(VarExpr ast, Object o) {
        ...
        if (ast.type.isArrayType()) {
            String name = (ast.V instanceof SimpleVar sv) ? sv.I.spelling : "";
            reporter.reportError(ErrorMessage.ARRAY_FUNCTION_AS_SCALAR.getMessage(),
                                 name, ast.position);
            // Keep ast.type as ArrayType (not errorType) so that downstream
            // operators (e.g., +) can detect the incompatibility and report *9.
        }
        return ast.type;
    }
    ...
    public Object visitFuncDecl(FuncDecl ast, Object o) {
        // Use declareVariable so redeclarations are caught and reported
    ...
    public Object visitGlobalVarDecl(GlobalVarDecl ast, Object o) {
        declareVariable(ast.I, ast);
        if (ast.T.isVoidType()) {
            reporter.reportError(ErrorMessage.IDENTIFIER_DECLARED_VOID.getMessage(),
                                 ast.I.spelling, ast.I.position);
            ...
            // *18: array declared without a size (e.g., int b[];)
            if (at.E instanceof EmptyExpr) {
                reporter.reportError(ErrorMessage.ARRAY_SIZE_MISSING.getMessage(),
                                     ast.I.spelling, ast.I.position);
            }
        }

        if (ast.E instanceof ArrayInitExpr aie) {
            if (!ast.T.isArrayType()) {
                // *14: array initialiser used for a scalar variable
                reporter.reportError(ErrorMessage.INVALID_INITIALISER_ARRAY_FOR_SCALAR.getMessage(),
                                     "", ast.E.position);
            } else {
                checkArrayInit(aie, (ArrayType) ast.T, ast.I.spelling, ast.position, o);
            }
        } else if (!(ast.E instanceof EmptyExpr)) {
            if (ast.T.isArrayType()) {
                // *15: scalar initialiser used for an array variable
    ...
    public Object visitLocalVarDecl(LocalVarDecl ast, Object o) {
        ...
        if (ast.T.isVoidType()) {
            ...
            // *18: array declared without a size (e.g., int x[];)
            if (at.E instanceof EmptyExpr) {
                reporter.reportError(ErrorMessage.ARRAY_SIZE_MISSING.getMessage(),
                                     ast.I.spelling, ast.I.position);
            }
    ...
    public Object visitIdent(Ident I, Object o) {
        ...
        binding.ifPresent(entry -> I.decl = entry.attr);
